...
**鱼池 CLI 修复** —— IDE 助手面板内可直接填写 `chat_ui` 表单、处理权限确认，不再无限等待；Agent 改文件后资源管理器与 Git 面板会可靠刷新。

## 🕘 v0.8.54 — 资源管理器

一键折叠/展开、标题栏吸顶、更紧凑的行高。

## 🕘 v0.8.53 — 权限即时生效 & 国际化

关闭 Shell/文件确认后立即作用于运行中会话；悬浮窗、设置、调试 i18n 与 Windows PPTX 改进。
