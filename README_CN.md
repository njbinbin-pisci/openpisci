...
**主会话锦鲤人设 + 输入区增强 + 鱼群导航** —— 可选锦鲤直接对话；多附件/技能；侧栏鱼群合并小鱼与锦鲤管理。

### 🐟 主会话
- **锦鲤人设模式**：输入区选择锦鲤后，以该锦鲤身份对话（独立 prompt、工具子集、记忆归属、可选 LLM 覆盖）。
- **多附件与技能**：待发送 chip 预览、Ctrl+V 粘贴图片、显式技能下拉。

### 🏫 鱼群
- 侧栏 **鱼群** 合并小鱼库与锦鲤管理（小鱼 | 锦鲤 双标签）；鱼池「管理锦鲤」跳转鱼群页。

### 🎨 界面
- 全站 `<select>` 样式统一（修复 Koi 编辑页模型下拉白底白字）。
