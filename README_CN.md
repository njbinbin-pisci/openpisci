...
**UI 统一与微信 IM 发文件** —— 全局深浅主题、紧凑控件规范，以及更清晰的微信文件发送提示词。

### 🎨 界面
- **深浅切换** —— OpenPiscis 标题旁太阳/月亮按钮；新增浅色主题；各页顶栏高度与背景统一。
- **紧凑控件** —— 日志筛选、聊天输入行统一 32px 下拉与图标按钮；上下文圆环去掉外框并缩小。
- **鱼池聊天** —— 去掉重复提示行，placeholder 承载完整说明；修复 i18n key 直出问题。

### 📎 微信 IM
- 系统提示词补充 `wechat` 频道能力与 `im_send_message(file_path=…)`、`SEND_FILE:` 跨平台路径示例。
