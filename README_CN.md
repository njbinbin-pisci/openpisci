...
**RobotZ 定位澄清与 UI 修补** —— 桌面自动化走内置工具；MCP 页只列外部服务器；鱼池 Git 图标统一为 SVG。

### 🤖 RobotZ
- **`robotz-automation` / `robotz-browser`** 编译进主程序，在「内置工具」页使用，**不在 MCP 页展示**。
- 启动时自动清理旧版 `robotz` MCP 配置项。

### 🎨 界面
- MCP 编辑服务器「传输方式」下拉改用统一 `select-control` 样式。
- 鱼池 Collab 右侧 Git 图标与 IDE 一致（SVG 源码管理图标）。
