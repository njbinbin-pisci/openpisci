...
pub async fn run_uia_drag_test(
    ...
    let prompt =
        "任务：将橙色小球拖拽到绿色目标区域。\n\
         \n\
         步骤：\n\
         1. 用 screen_capture 工具截取全屏截图（action=capture, grid=true）\n\
         2. 仔细观察截图中的坐标网格（每200像素一条线，标签显示绝对屏幕坐标）\n\
         3. 找到橙色圆形小球的中心坐标（读取最近的网格线标签，精确估算）\n\
         4. 找到绿色虚线矩形（目标区域）的中心坐标（读取最近的网格线标签，精确估算）\n\
         5. 用 uia 工具的 drag_drop 操作，从小球中心拖拽到目标区域中心\n\
         \n\
         重要提示：\n\
         - 网格标签显示的就是屏幕绝对坐标，可直接用于 uia drag_drop\n\
         - 读取坐标时，先找最近的网格线，再根据元素与网格线的相对位置微调\n\
         - 橙色小球是一个橙色圆形，直径约40像素\n\
         - 目标区域是一个绿色虚线矩形，约120x120像素，位于测试区域右侧\n\
