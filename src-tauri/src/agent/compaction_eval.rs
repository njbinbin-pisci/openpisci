...
use crate::agent::loop_::{
    build_request_messages, CTX_FULL_TURNS,
};
...
use crate::{
    agent::compaction::CTX_KEEP_RECENT_TOOL_CARRIERS,
    commands::chat::{collapse_superseded_tool_failures, sanitize_tool_use_result_pairing},
};
...
fn print_report(session: &Session, stats: &SessionStats) {
    ...
    println!(
        ...
        stats.forced_demote_request_tokens, forced_ratio * 100.0
    );
    println!(
    ...
    if stats.empty_tool_use_ids
        + stats.duplicate_tool_use_ids
        + stats.tool_use_id_orphans
        > 0
    {
        println!(
    ...
    rows.sort_by(|a, b| b.1.demoted.cmp(&a.1.demoted).then(b.1.total.cmp(&a.1.total)));
    if !rows.is_empty() {
...
fn eval_real_sessions() {
    ...
    if let Ok(raw) = std::env::var("PISCI_EVAL_MIN_REDUCTION_PCT") {
        let min_pct: f64 = raw.parse().expect(
            "PISCI_EVAL_MIN_REDUCTION_PCT must be a number, e.g. \"30\" for 30%",
        );
        assert!(agg_full > 0, "no sessions evaluated — cannot assert reduction");
